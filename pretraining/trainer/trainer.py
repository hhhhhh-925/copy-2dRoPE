...
class LMTrainer:
    ...
    def get_validation_loss(
        ...
        self.accelerator.print(f"Validation loss: {val_loss}")
        return val_loss
    ...
    def handle_resumption(self, data_iterator):
        ...
        if self.args.resume_path is not None and self.args.resume_step is not None:
            ...
            if self.args.reverse_dataset_order:
                self.accelerator.print(
                    "reverse_dataset_order=1: not skipping dataloader batches; "
                    f"cur_step set to {self.args.resume_step}."
                )
            else:
