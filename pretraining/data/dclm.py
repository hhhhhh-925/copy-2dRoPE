...
def _dclm_jsonl_shard_paths(data_dir: Union[str, Path]) -> List[Path]:
    """Sorted shard paths; same ordering as HuggingFace path-based load for this layout."""
    root = Path(data_dir).resolve()
    return sorted(root.rglob("*.jsonl.zst"))
...
def build_dataset(
    ...
    **kwargs,
    ...
    If ``reverse_shard_order`` is True, shards (``*.jsonl.zst``) are iterated in the
    opposite order of the default layout—useful to resume training on fresh shards
    without skipping the entire forward stream (pair with ``resume_step=0`` in the
    trainer so the dataloader is not advanced batch-by-batch).
    """
    ...
    if reverse_shard_order:
        shard_paths = _dclm_jsonl_shard_paths(data_dir)
        if not shard_paths:
            raise FileNotFoundError(
                f"No *.jsonl.zst shards under {data_dir!r} (needed for reverse_shard_order)."
            )
        shard_paths.reverse()
        raw_dataset = load_dataset(
            "json",
            data_files={"train": [str(p) for p in shard_paths]},
            split="train",
            streaming=streaming,
        )
    else:
